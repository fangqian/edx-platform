...
class CommentViewSetListTest(DiscussionAPIViewTestMixin, ModuleStoreTestCase, ProfileImageTestMixin):
    ...
    def test_profile_image_requested_field_null_endorsed(self):
        """
        Tests if 'endorsed' is True but 'endorsed_by' is null; the api do not crash.
        This tests is for any old/stale data in prod/stage environments.
        """
        self.register_get_user_response(self.user)
        thread = self.make_minimal_cs_thread({
            "thread_type": "question",
